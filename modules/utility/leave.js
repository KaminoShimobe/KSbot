...
module.exports = {
	name: 'leave',
	description: 'Leave a KSRPG dungeons',
    ...
    con.query(`SELECT * FROM user WHERE id = '${message.author.id}'`, (err, rows) => {
		...
		con.query(`SELECT * FROM ksrpg WHERE id = '${message.author.id}'`, (err, rows) => {		
			...
			if(Battling.has(message.author.id)){
				message.author.send("You are in a battle right now! You cannot leave!")
				return;
			}

			if(PvP.has(message.author.id)){
				message.author.send("You are in a PVP match right now! You cannot leave!")
				return;
			}
			...
			sql = `UPDATE ksrpg SET location = '', turn = ${0} WHERE id = '${message.author.id}'`
         	con.query(sql);
			message.author.send("You left the dungeon!")
