...
other.send(`Respond with **rock**, **paper**, **scissors**, or **random** to use against ` + mName + ` \n (r, p, s, rand for short)`)
			const collectorr = new other.createMessageCollector(m => m.author.id === other.id, { time: 100000000 });
		...
		function duel(){
			...
			const collector = new them.createMessageCollector(m => m.author.id === message.author.id, { time: 100000000 });
	        		collector.once('collect', message => {
	            		if (message.content == `rock` || message.content == `r`) {
