   ...
   con.query(`SELECT * FROM garden WHERE owner = '${other.id}' AND id = '${message.guild.id}'`, (err, rows) => {
        ...
        con.query(`SELECT * FROM plant WHERE owner = '${message.author.id}' AND id = '${message.guild.id}'`, (err, rows) => {
              ...
              if(rows[trade1-1] == undefined){
               ...
               sql = `UPDATE plant SET type = '${type}' status = '${stage}', health = ${200}, hexcolor = ${petals} WHERE owner = '${other.id}' AND id = '${message.guild.id}' AND hexcolor = '${petals2}'`;
               con.query(sql);
               sql2 = `UPDATE plant SET type = '${type2}' status = '${stage2}', health = ${200}, hexcolor = ${petals2} WHERE owner = '${message.author.id}' AND id = '${message.guild.id}' AND hexcolor = '${petals}'`;
               con.query(sql2);
